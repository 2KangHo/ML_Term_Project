    ...
    def build_net(self):
        with tf.variable_scope(self.name) as scope:
            ...
            with tf.name_scope('conv2_layer') as inner_scope:
                ...
                #    Conv     -> (?, 28, 28, 32)
                #    ReLU     -> (?, 28, 28, 32)
                W2 = tf.Variable(tf.random_normal([3, 3, 32, 32], stddev=0.01), name='weight2')
            ...
            with tf.name_scope('conv4_layer') as inner_scope:
                # L4 ImgIn shape=(?, 14, 14, 128)
                #    Conv     -> (?, 14, 14, 128)
                #    ReLU     -> (?, 14, 14, 128)
                W4 = tf.Variable(tf.random_normal([3, 3, 128, 128], stddev=0.01), name='weight4')
            ...
            with tf.name_scope('conv6_layer') as inner_scope:
                # L6 ImgIn shape=(?, 7, 7, 512)
                #    Conv     -> (?, 7, 7, 512)
                #    ReLU     -> (?, 7, 7, 512)
                W6 = tf.Variable(tf.random_normal([3, 3, 512, 512], stddev=0.01), name='weight6')
...
writer = tf.summary.FileWriter(cfg.log_dir + '/m5')
model = Model(sess, "m5")
...
plt.ylabel('cost')
plt.xlabel('epochs')
plt.title("CReLU&ReLU Model Costs")
plt.show()

plt.plot(np.squeeze(train_accu))
plt.ylabel('train accuracy')
plt.xlabel('epochs')
plt.title("CReLU&ReLU Model Train accuracy")
plt.show()
