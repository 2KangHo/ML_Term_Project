    ...
    def build_net(self):
        with tf.variable_scope(self.name) as scope:
            ...
            with tf.name_scope('conv4_layer') as inner_scope:
                ...
                #    Conv     -> (?, 14, 14, 128)
                #    ReLU6     -> (?, 14, 14, 128)
                W4 = tf.Variable(tf.random_normal([3, 3, 64, 128], stddev=0.01), name='weight4')
            ...
            with tf.name_scope('conv6_layer') as inner_scope:
                ...
                #    Conv     -> (?, 7, 7, 512)
                #    ReLU6     -> (?, 7, 7, 512)
                W6 = tf.Variable(tf.random_normal([3, 3, 256, 512], stddev=0.01), name='weight6')
...
writer = tf.summary.FileWriter(cfg.log_dir + '/m3')
model = Model(sess, "m3")
...
plt.ylabel('cost')
plt.xlabel('epochs')
plt.title("ReLU&ReLU6 Model Costs")
plt.show()

plt.plot(np.squeeze(train_accu))
plt.ylabel('train accuracy')
plt.xlabel('epochs')
plt.title("ReLU&ReLU6 Model Train accuracy")
plt.show()
